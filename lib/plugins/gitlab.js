...
class GitLab {
  /**
   *Creates an instance of GitLab.
   * @param {{protectedKeys: string, url: string}} settings
   * @memberof GitLab
   */
  ...
  createOrUpdateKey (metadata, key) {
    const { group, project } = this.parseMetadata(metadata);
    let projectId = -1;
  ...
  parseMetadata (metadata) {
    ...
    // project
    const group = projectPath.slice(0, -1).join('/'),
